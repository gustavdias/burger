...
class ContactData extends Component {
  ...
  orderHandler = (event) => {
    ...
    const order = {
      ...
      orderData: formData,
      userId: this.props.userId, //!Displaying user specific orders
    };

    this.props.onOrderBurger(order, this.props.token); //dispatches action creator for purchaseBurger
    //!token for purchaseBurger
...
const mapStateToProps = (state) => {
  return {
    ...
    token: state.auth.token, //!token for purchaseBurger
...
const mapDispatchToProps = (dispatch) => {
  return {
    onOrderBurger: (orderData, token) =>
      dispatch(orderActions.purchaseBurger(orderData, token)), //!token for purchaseBurger
  };
};
